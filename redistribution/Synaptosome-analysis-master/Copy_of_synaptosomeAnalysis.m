...
directory = 'D:\PostDoc Cambridge\Wei\CD\reconstruct image';

% path to folder where outputfolder will be created (if doesn't already exist)
output_dir = fullfile('D:\PostDoc Cambridge\Wei\CD\reconstruct image\Results_IntDen600_Unc400_Sigma100_200',filesep);
...
max_sigma       = 200;  % maximum sigma value of the Gaussian fit
%min_intensity   = 500; % min number of photons expected for an event
max_uncertainty = 400;   % max uncertainty expected for single molecules in focus
min_uncertainty = 0;    % max uncertainty expected for single molecules in focus 

%min_intensity_density = 0.02; %photon/nm^2

Nloc = [600 30]; %total number of localizations per 10 frames in each channel
...
CustomColors = 1; %Put 1 if you want to paint your image in a specific colors 
Colors = [0 1 1; 1 0 1]; %Specify rgb code for all colors you want to use (should be N_channels x 3)
