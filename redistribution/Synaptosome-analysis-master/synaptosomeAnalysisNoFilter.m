...
directory = 'D:\PostDoc Cambridge\STORM\Data\2020_06_26_PC12_Pores_Membrane(647)_Asyn(594)\StimBPB2bot\Analysis';

% path to folder where outputfolder will be created (if doesn't already exist)
output_dir = fullfile('D:\PostDoc Cambridge\STORM\Data\2020_06_26_PC12_Pores_Membrane(647)_Asyn(594)\StimBPB2bot\Analysis\Results_NoFilter',filesep);

repeat           = '4Cb';
condition        = 'phys';
%Specify here channels names that are used for file namings (could be one or more channels)
channel_token    = {'_647', '_568_reg'};
...
max_radius     = [0; 0]; % maximum radius of clusters in all chanels respectively (nm) (should be the same size as channel_token)
min_nr_locs    = [0; 0];  % minimum nr of locs within min_radius (should be the same size as channel_token)

P              = [0; 0]; % blobs with fewer pixels will be removed from mask (should be the same size as channel_token)


level          = [0; 0] % threshold (0-1) for mask (0 for Otsu's threshold) (should be the same size as channel_token)
RefCh          = 1;     %number of the channel which is a reference channel for synaptosome analysis
...
filter          = 0;    % to filter the localisation files before further processing based on the following parameters
min_frames      = 1;  % throw away initial frames to avoid really high density data
min_sigma       = 0;   % minimum sigma value of the Gaussian fit
max_sigma       = 500;  % maximum sigma value of the Gaussian fit
...
min_uncertainty = 0;    % min uncertainty expected for single molecules in focus 

%min_intensity_density = 0.02; %photon/nm^2

Nloc = [2000 1000]; %total number of localizations per 10 frames in each channel
essence = 0; % 1 to only write away files essential for further analysis
SynaptosomeAnalysis = 0; %1 is you want to perform analysis of intersection of synaptosomes from different channels
CustomColors = 0; %Put 1 if you want to paint your image in a specific colors 
Colors = [0 1 1; 1 1 0]; %Specify rgb code for all colors you want to use (should be N_channels x 3)
...
for i = 1:size(filelist,1)
    ...
    %Stas 06.11.19
    for nch = 1:N_channel  
        %introducing new parameter: density of intensity
        IntDen = locs(nch).channel.intensity./(pi*(2.*locs(nch).channel.sigma).^2);
        locs(nch).channel.IntensityDensity = IntDen; 
        
        %find treshold values for this parameter based on visual inspection
        %of blinks for 10 random frames
        Nframes = locs(nch).channel.frame(end);
        min_intensity_density(nch) = FindTreshold(IntDen, Nframes/10*Nloc(nch), 0.0001)
%        min_intensity_density(nch) = 0;
        disp(['Automatic treshold chosen for density intensity for ' num2str(nch) ' channel is ' num2str(min_intensity_density(nch)) ])
    end
...
%     of the Fov in one of the three channels
    ...
    if remove_border        
        ...
        for nch=1:N_channel
            locs_filtered(nch).channel = locs(nch).channel(locs(nch).channel.x>border_width,:);
            locs_filtered(nch).channel = locs_filtered(nch).channel(locs_filtered(nch).channel.x<Fov*pixelsize-border_width,:);
            locs_filtered(nch).channel = locs_filtered(nch).channel(locs_filtered(nch).channel.y>border_width,:);
            locs_filtered(nch).channel = locs_filtered(nch).channel(locs_filtered(nch).channel.y<Fov*pixelsize-border_width,:);
        end
...
%     
    ...
    if filter
        ...
        for nch = 1:N_channel
            ...
            %Stas 06.11.19
            locs_filtered(nch).channel = locs_filtered(nch).channel(locs_filtered(nch).channel.IntensityDensity> min_intensity_density(nch) ,:);
            ...
            locs_filtered(nch).channel = locs_filtered(nch).channel(locs_filtered(nch).channel.uncertainty       < max_uncertainty ,:);
              ...
              %if ~strcmp(format_for_filtering,'rapidstorm')
                locs_filtered(nch).channel = locs_filtered(nch).channel(locs_filtered(nch).channel.sigma   > min_sigma,:);
                locs_filtered(nch).channel = locs_filtered(nch).channel(locs_filtered(nch).channel.sigma   < max_sigma,:);
            %end
        end
...
%     %% Write away filtered localisations
      ...
      for nch=1:N_channel 
          writeLocFile(locs_filtered(nch).channel,fullfile(path_output,strcat(area_token,['_locs_' num2str(nch) '_channel_filtered.csv'])),format);   
      ...
      if N_channel>1
          for nch1 = 1:N_channel-1
              for nch2 = nch1+1:N_channel
                    ...
                    if CustomColors == 1
                            merged_reconstruction(:,:,1) = [uint8(img(nch1).channel*Colors(1,1)+img(nch2).channel*Colors(2,1))];
                            merged_reconstruction(:,:,2) = [uint8(img(nch1).channel*Colors(1,2)+img(nch2).channel*Colors(2,2))];
                            merged_reconstruction(:,:,3) = [uint8(img(nch1).channel*Colors(1,3)+img(nch2).channel*Colors(2,3))];
                    else
...
%     
    ...
    for nch = 1:N_channel
        ...
        if max_radius(nch)~= 0 && min_nr_locs(nch) ~= 0
            ...
            %save localizations after Filter Density
            locs_density_filtered_full = locs_filtered(nch).channel(indeces(nch).channel==1,:);
            filename_dens_filt = char(strcat(area_token,['_locs_' num2str(nch) '_channel_density_filtered.csv']));
            writetable(locs_density_filtered_full,fullfile(path_output, filename_dens_filt));
            clear locs_density_filtered_full
            
              % Get two-colour reconstruction red-green for filtered data(only for visualization)
              if N_channel>1
                  for nch1 = 1:N_channel-1
                      for nch2 = nch1+1:N_channel
                            merged_reconstruction = uint8(zeros(size(img(nch1).channel,1),size(img(nch2).channel,2),3))
                            ...
                            filename{nch} = char(strcat(area_token,['_reconstruction_of_' num2str(nch1) '_and_' num2str(nch2) '_channel.tif']));
                            if ~essence; imwrite(flip(flip(merged_reconstruction,2),1), fullfile(path_output, filename{nch})); end
                            if show; figure; imshow(flip(flip(merged_reconstruction,2),1)); title(['Reconstruction of filtered data for ' num2str(nch1) ' and ' num2str(nch2) ' channel']); end
        %     
                      end
                  end
              end
...
%     
    % Binarize image to get a mask in all channels
     for nch = 1:N_channel   
        ...
        if ~essence
            filename1{nch} = strcat(area_token,'_detected_synaptosomes.tif');
            imwrite(flip(flip(mask(nch).channel,2),1), fullfile(path_output,filename1{nch}));
        end
     end
    ...
    %     % Get number of potential synaptosomes detected
        ...
        if numberOfClusters > 0
            ...
            for nch=1:N_channel 
                ...
                Names{N_Names} = ['Overlap_with_' num2str(nch) '_channel']; 
                Names{N_Names+1} = ['Weighted_overlap_with_' num2str(nch) '_channel'];  
                N_Names = N_Names+2;
                end
            end
