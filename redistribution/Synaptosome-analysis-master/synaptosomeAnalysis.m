...
directory = 'D:\PostDoc Cambridge\DNA_PAINT\DATA\2020_10_08_Stas_Marius_DNA_PAINT_synaptosomes_phys\a4\Analysis';

% path to folder where outputfolder will be created (if doesn't already exist)
output_dir = fullfile('D:\PostDoc Cambridge\DNA_PAINT\DATA\2020_10_08_Stas_Marius_DNA_PAINT_synaptosomes_phys\a4\Analysis\Results_sigma_100_250_unc_5_40_IntDen_50_50',filesep);
...
max_radius     = [0; 0; 0]; % maximum radius of clusters in all chanels respectively (nm) (should be the same size as channel_token)
min_nr_locs    = [0; 0; 0];  % minimum nr of locs within min_radius (should be the same size as channel_token)
...
RefCh          = 2;     %number of the channel which is a reference channel for synaptosome analysis
...
min_sigma       = 100;   % minimum sigma value of the Gaussian fit
max_sigma       = 250;  % maximum sigma value of the Gaussian fit
...
min_uncertainty = 5;    % min uncertainty expected for single molecules in focus 

%min_intensity_density = 0.02; %photon/nm^2

Nloc = [50 50]; %total number of localizations per 10 frames in each channel
