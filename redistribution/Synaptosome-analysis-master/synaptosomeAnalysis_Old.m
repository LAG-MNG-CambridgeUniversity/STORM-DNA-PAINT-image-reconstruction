...
directory = 'D:\PostDoc Cambridge\STORM\Data\2019_11_11_Stas_Andrew_Neurons\Contr1\Analysis';

% path to folder where outputfolder will be created (if doesn't already exist)
output_dir = fullfile('D:\PostDoc Cambridge\STORM\Data\2019_11_11_Stas_Andrew_Neurons\Contr1\Analysis\ResultsIntDen0.02',filesep);
...
min_sigma       = 140;   % minimum sigma value of the Gaussian fit
max_sigma       = 260;  % maximum sigma value of the Gaussian fit
%min_intensity   = 200000; % min number of photons expected for an event
    ...
    if filter
        ...
        for nch = 1:N_channel
           ...
           % locs_filtered(nch).channel = locs_filtered(nch).channel(locs_filtered(nch).channel.intensity       > min_intensity ,:);
            ...
            %Stas 06.11.19
            locs_filtered(nch).channel = locs_filtered(nch).channel(locs_filtered(nch).channel.IntensityDensity> min_intensity_density ,:);
