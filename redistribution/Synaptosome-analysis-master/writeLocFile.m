...
if strcmp(format,'thunderstorm')
    fid = fopen(path,'wt');
    if dim == 2
        header = 'frame,x [nm],y [nm],sigma [nm],intensity [photon],offset [photon],bkgstd [photon],uncertainty [nm],Intensity Density\n';
        fprintf(fid,header);
        if fid > 0
    ...
    elseif dim == 3
        header = 'frame,x [nm],y [nm],z [nm],sigma [nm],intensity [photon],offset [photon],bkgstd [photon],uncertainty [nm],Intensity Density\n';
        fprintf(fid,header);
        if fid > 0
