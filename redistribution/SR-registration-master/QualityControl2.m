...
correctedLocFile=[];deletedLocalizations=[];
while n<size(localizations,1)
   ...
   if localizations(n,2)==Current_Frame 
      ...
      if isempty(correctedLocFileOneFrame)==0
       ...
       if num_TooClose>0
           deletedLocalizationsOneFrame = Coord_Frame;
           NearNeighbour = num_TooClose == 0
           deletedLocalizationsOneFrame(NearNeighbour,:) = [];
           deletedLocalizations = [deletedLocalizations; deletedLocalizationsOneFrame];
       end
      Coord_Frame=[];
   end
...
% Plot
if show
    if size(deletedLocalizations,1)>1
